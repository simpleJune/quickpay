...
const fs = require('fs')
const demoPath = path.resolve(__dirname, '../src/list.json')
  ...
  plugins: [
    ...
    },
    {
      ...
      fn: function (source) {
        ...
        list.forEach(one => {
          let filename = one
          let path = `/components/${toDash(one)}`
          if (/#/.test(one)) {
          ...
          str.push(`{
            path: '${path}',
            components: () => import('./src/${filename}.vue').then(m => m.default)
          }`)
        })
